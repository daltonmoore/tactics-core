...
namespace TacticsCore.Editor.Tutorials.ListView
{
    ...
    {
        [MenuItem("Planets/Multi Column List")]
        static void Summon()
        {
            GetWindow<PlanetsMultiColumnListView>("Multi Column Planet List");
        }
        ...
        void CreateGUI()
        {
            ...
            listView.columns["name"].bindCell = (VisualElement element, int index) =>
                (element as Label).text = units[index].name;
            listView.columns["populated"].bindCell = (VisualElement element, int index) =>
                (element as Toggle).value = units[index].populated;
        }
    }
}